package es.caib.interdoc.api.interna.secured.exempleenumsecuritzat;
...
import es.caib.interdoc.api.interna.all.exemplepublic.ExemplePojo;
...
public class ExempleEnumSecuritzatService {
    ...
    protected Logger log = LoggerFactory.getLogger(ExempleEnumSecuritzatService.class);
    ...
    @Path("/versio")
	@GET
	@RolesAllowed({ Constants.ITD_ADMIN, Constants.ITD_USER })
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @Operation(
            tags = "Versió",
    ...
    public Response versio(
        ...
        try {
        	...
        	log.info("secured/exempleenum/versio");
            ExemplePojo exemple = new ExemplePojo(version.getVersion() + "_" + idioma);
            log.info("Version => " + exemple.getVersio());
