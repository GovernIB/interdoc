...
@Named
@ViewScoped
public class ListReferenciaXML extends AbstractController implements Serializable {

    private static final long serialVersionUID = -6015369276336087696L;

    private static final Logger LOG = LoggerFactory.getLogger(ListReferenciaXML.class);

    @EJB
    private ReferenciaXMLServiceFacade referenciaService;

    /**
     * Model de dades emprat pel compoment dataTable de primefaces.
     */
    private LazyDataModel<ReferenciaXMLDTO> lazyModel;

    public LazyDataModel<ReferenciaXMLDTO> getLazyModel() {
        return lazyModel;
    }
    ...
    public void init() {
        ...
        lazyModel = new LazyDataModel<ReferenciaXMLDTO>() {
            ...
            */
            ...
            @Override
            public List<ReferenciaXMLDTO> load(int first, int pageSize, Map<String, SortMeta> sortBy,
                                               Map<String, FilterMeta> filterBy) {
                LOG.info("filterBy: {}", filterBy);

                // Dins JSF emprarem noms que coincideixin amb els valors de l'enumeració AtributUnitat
                Map<ReferenciaXMLAtribut, Object> filter = PFUtils.filterMetaToFilter(ReferenciaXMLAtribut.class, filterBy);
                List<Ordre<ReferenciaXMLAtribut>> ordenacions = PFUtils.sortMetaToOrdre(ReferenciaXMLAtribut.class, sortBy);

                Pagina<ReferenciaXMLDTO> pagina = referenciaService
