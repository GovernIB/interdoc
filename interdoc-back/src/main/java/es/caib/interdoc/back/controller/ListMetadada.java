...
@Named
@ViewScoped
public class ListMetadada extends AbstractController implements Serializable {

    private static final long serialVersionUID = -6015369276336087696L;

    private static final Logger LOG = LoggerFactory.getLogger(ListMetadada.class);
     ...
     */
    private LazyDataModel<MetadadaDTO> lazyModel;

    public LazyDataModel<MetadadaDTO> getLazyModel() {
        return lazyModel;
    }
    ...
    public void init() {
        ...
        lazyModel = new LazyDataModel<MetadadaDTO>() {
            ...
            */
            ...
            @Override
            public List<MetadadaDTO> load(int first, int pageSize, Map<String, SortMeta> sortBy,
                                          Map<String, FilterMeta> filterBy) {
                LOG.info("filterBy: {}", filterBy);

                // Dins JSF emprarem noms que coincideixin amb els valors de l'enumeració AtributUnitat
                Map<MetadadaAtribut, Object> filter = PFUtils.filterMetaToFilter(MetadadaAtribut.class, filterBy);
                List<Ordre<MetadadaAtribut>> ordenacions = PFUtils.sortMetaToOrdre(MetadadaAtribut.class, sortBy);

                Pagina<MetadadaDTO> pagina = metadadaService
