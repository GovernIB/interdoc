...
@Named
@ViewScoped
public class ListPeticio extends AbstractController implements Serializable {

    private static final long serialVersionUID = -6015369276336087696L;

    private static final Logger LOG = LoggerFactory.getLogger(ListPeticio.class);
     ...
     */
    private LazyDataModel<PeticioATercerDTO> lazyModel;

    public LazyDataModel<PeticioATercerDTO> getLazyModel() {
        return lazyModel;
    }
    ...
    public void init() {
        ...
        lazyModel = new LazyDataModel<PeticioATercerDTO>() {
            ...
            */
            ...
            @Override
            public List<PeticioATercerDTO> load(int first, int pageSize, Map<String, SortMeta> sortBy,
                                                Map<String, FilterMeta> filterBy) {
                LOG.info("filterBy: {}", filterBy);

                // Dins JSF emprarem noms que coincideixin amb els valors de l'enumeració AtributUnitat
                Map<PeticioATercerAtribut, Object> filter = PFUtils.filterMetaToFilter(PeticioATercerAtribut.class, filterBy);
                List<Ordre<PeticioATercerAtribut>> ordenacions = PFUtils.sortMetaToOrdre(PeticioATercerAtribut.class, sortBy);

                Pagina<PeticioATercerDTO> pagina = peticioService
